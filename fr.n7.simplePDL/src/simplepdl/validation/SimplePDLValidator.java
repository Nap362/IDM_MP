...
public class SimplePDLValidator extends SimplepdlSwitch<Boolean> {
	...
	public Boolean caseWorkSequence(WorkSequence object) {
		...
		this.result.recordIfFailed(
				object.getPredecessor().getLinksToSuccessors().stream()
					.filter(p -> p.getLinkType().equals(object.getLinkType()) && p.getSuccessor().equals(object.getSuccessor()))
					.count() == 1,
				object, 
				"Il y a plusieurs dépendances identiques");
		return null;
	}
	...
	public Boolean caseGuidance(Guidance object) {
		this.result.recordIfFailed(
				!object.getText().isEmpty(), 
				object,
				"Le texte de cette guidance est vide");
		return null;
	}
